    ...
    <>
      <style>{`
        @media print {
          ...
          #invoice-print .print\\:hidden { display: none !important; }
        }
      `}</style>
      ...
      >
        <div
          className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[95vh]"
          onClick={e => e.stopPropagation()}
        >
          {/* ── Toolbar ── */}
          <div className="px-4 py-3 border-b border-slate-100 flex justify-between items-center bg-slate-50 rounded-t-2xl print:hidden shrink-0">
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 rounded-lg bg-[#0D2B5E] flex items-center justify-center">
                <ShieldCheck className="text-white w-4 h-4" />
              </div>
              <span className="font-bold text-[#0D2B5E] text-sm">Tax Invoice — {order.invoice_no || 'DRAFT'}</span>
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => window.print()}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-[#0D2B5E] text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-[#0a2147] transition-colors"
              >
                <Printer size={14} /> Print (A5)
              </button>
              <button onClick={onClose} className="p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-200 rounded-full transition-all">
                <X size={18} />
              </button>
            </div>
          </div>
          ...
          {/* ── Invoice Body ── */}
          <div id="invoice-print" className="overflow-y-auto print:overflow-visible">
            <div className="border border-slate-200 m-4 rounded-xl overflow-hidden print:border print:m-0 print:rounded-none shadow-sm">
