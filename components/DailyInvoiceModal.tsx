...
export const DailyInvoiceModal: React.FC<DailyInvoiceModalProps> = ({ orders, retailer, date, onClose, variant = 'drawer' }) => {
    ...
    const handleDownloadPdf = useCallback(() => {
        ...
        downloadPdf({
            elementId: 'daily-invoice-print',
            filename: `Daily-Invoice-${date}-${retailer.name.replace(/\s+/g, '_')}.pdf`,
        })
            .then(() => setIsDownloading(false))
            .catch((err) => { console.error('Daily PDF failed:', err); setIsDownloading(false); });
    }, [date, retailer]);
    ...
    // ── Toolbar (shared between drawer & inline) ──
    const toolbar = (
    ...
    // ── Invoice body (the printable content) ──
    const invoiceBody = (
    ...
    if (variant === 'inline') {
        return (
            <div className="bg-white rounded-xl border border-slate-200 shadow-lg overflow-hidden">
                {toolbar}
                <div className="overflow-y-auto bg-slate-50 p-3 sm:p-4 flex justify-center w-full">
                    {invoiceBody}
                </div>
            </div>
        );
