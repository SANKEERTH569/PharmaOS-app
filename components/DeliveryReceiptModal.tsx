...
export const DeliveryReceiptModal: React.FC<DeliveryReceiptModalProps> = ({ order, retailer, onClose, variant = 'modal' }) => {
   ...
   const handleDownloadPdf = useCallback(() => {
      ...
      downloadPdf({
         elementId: 'delivery-receipt-print',
         filename: `Challan-${order.id.slice(-6)}.pdf`,
         margin: [8, 8, 8, 8],
      })
         .then(() => setIsDownloading(false))
         .catch((err) => { console.error('Challan PDF failed:', err); setIsDownloading(false); });
   }, [order]);

   const handlePrint = useCallback(() => {
      const el = document.getElementById('delivery-receipt-print-container');
      if (!el) return;
   ...
   const ITEMS_PER_PAGE = 36; // 12 rows x 3 columns
   const pages = [];
   ...
   return (
      <div
         className={isInline ? "w-full animate-in fade-in duration-200" : "fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[60] p-4 animate-in fade-in duration-200"}
         onClick={isInline ? undefined : onClose}
      >
         <div
            className={isInline ? "bg-white rounded-2xl shadow-sm border border-slate-200 w-full flex flex-col" : "bg-white rounded-2xl shadow-2xl w-full max-w-3xl flex flex-col max-h-[90vh]"}
         ...
         >
            ...
            <div className={isInline ? "p-6 bg-slate-50 rounded-b-2xl" : "p-6 overflow-y-auto bg-slate-50 rounded-b-2xl"}>
               <div id="delivery-receipt-print-container" className="flex flex-col gap-8">
                  {pages.map((pageItems, pageIndex) => (
                     <div key={pageIndex} className="print-page bg-white border border-emerald-100 p-6 rounded-xl relative box-border flex flex-col" style={{ minHeight: isInline ? 'auto' : '297mm' }}>
                        {/* Header */}
                        <div className="flex justify-between items-center mb-6 pb-4 border-b-2 border-emerald-500 shrink-0">
                           ...
                           <div className="text-right">
                              ...
                              <p className="text-[10px] font-bold text-slate-500 mt-1">Page {pageIndex + 1} of {pages.length}</p>
                           </div>
                        </div>

                        {/* Shipping Info (Only on first page) */}
                        {pageIndex === 0 && (
                        ...
                        <div className="mb-6 flex-1">
                           <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                              {pageIndex === 0 ? <><Hash size={12} /> Consignment Contents Checklist</> : <><Hash size={12} /> Consignment Contents (Continued)</>}
                           </h3>
                           <div className="grid grid-cols-3 gap-3">
                              {pageItems.map((item, idx) => (
                                 <div key={item.id} className="flex items-center justify-between border border-slate-200 rounded p-2 bg-white">
                                    <div className="flex gap-2 items-center overflow-hidden">
                                       <span className="text-slate-400 font-bold text-[10px] w-4 shrink-0">
                                          {pageIndex * ITEMS_PER_PAGE + idx + 1}.
                                       </span>
                                       <div className="overflow-hidden">
                                          <p className="font-black text-slate-900 text-[10px] uppercase truncate" title={item.medicine_name}>
                                             {item.medicine_name}
                                          </p>
                                          <p className="text-[9px] text-slate-500 font-medium italic mt-0.5">
                                             Qty: <span className="text-emerald-700 font-black text-[10px] ml-0.5">{item.qty}</span>
                                          </p>
                                       </div>
                                    </div>
                                    <div className="w-5 h-5 border-2 border-slate-200 rounded shrink-0 ml-2"></div>
                                 </div>
                              ))}
                           </div>
                        </div>

                        {/* Confirmation Area (Only on last page) */}
                        <div className="mt-auto pt-6">
                           {pageIndex === pages.length - 1 && (
                              <>
                                 <div className="grid grid-cols-2 gap-8 border-t-2 border-emerald-500 pt-6">
                                    <div>
                           ...
                           {pageIndex < pages.length - 1 && (
                              <div className="text-center text-slate-400 text-xs italic">Continued on next page...</div>
                           )}
                        </div>
                     </div>
                  ))}
