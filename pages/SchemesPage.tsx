...
import {
    Plus, Tag, Trash2, Search, ArrowLeft,
...
import { Scheme, SchemeType, Medicine } from '../types';

export const SchemesPage = () => {
    const { schemes, medicines, fetchSchemes, createScheme, deleteScheme } = useDataStore();
    ...
    useEffect(() => {
        if (wholesaler?.id) {
            setLoading(true);
            fetchSchemes().finally(() => setLoading(false));
        }
    }, [wholesaler]);

    const filteredMeds = medicines.filter(m =>
        m.is_active && m.name.toLowerCase().includes(searchMed.toLowerCase())
    ).slice(0, 10);
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-500 mt-1">Manage rules that automatically apply during Quick Sale</p>
                    </div>
                </div>
                {!showAdd && (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-5">
                                ...
                                <div>
                                    ...
                                    <input type="text" required placeholder="e.g. Dolo 10+1 Summer Promo"
                                ...
                                {(type === 'BOGO' || type === 'HALF_SCHEME') && (
                                    <div className="space-y-4 animate-in fade-in duration-200">
                                        <div className="relative">
                                            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">Target Medicine</label>
                                            <input type="text" placeholder="Search medicines..." required={!medicineId}
                                                className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none text-sm font-medium"
                                                value={medicineId ? selectedMed?.name || '' : searchMed}
                                                ...
                                                <div className="absolute z-10 w-full mt-1 bg-white border border-slate-200 rounded-xl shadow-lg max-h-48 overflow-y-auto">
                                                    {filteredMeds.map(m => (
                                                        <button key={m.id} type="button" onClick={() => { setMedicineId(m.id); setSearchMed(''); setShowMedDropdown(false); }}
                                                            ...
                                                            {m.name} <span className="text-xs text-slate-400 ml-1">{m.brand}</span>
                                                        </button>
                                                    ))}
                                                </div>
                                            )}
                                        ...
                                        <div className="flex items-start gap-2 text-[11px] text-indigo-600 bg-indigo-50 p-3 rounded-lg border border-indigo-100">
                                            ...
                                            <p><strong>Example:</strong> For every <span className="font-bold">{minQty || 'X'}</span> bought, the buyer receives a discount equivalent to the price of <span className="font-bold">{freeQty || 'Y'}</span> units.</p>
                                        </div>
                                    </div>
                                )}
                                ...
                                {type === 'CASH_DISCOUNT' && (
                                    <div className="space-y-4 animate-in fade-in duration-200">
                                        ...
                                        <div className="flex items-start gap-2 text-[11px] text-amber-700 bg-amber-50 p-3 rounded-lg border border-amber-100">
                                            ...
                                            <p><strong>Note:</strong> Cash discounts apply universally to the entire order subtotal during checkout.</p>
                                        </div>
                                    </div>
                                )}
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200">
                ...
                {schemes.length === 0 && !loading ? (
                    <div className="p-12 text-center text-slate-500">
                        ...
                        <p className="text-xs mt-1">Create a scheme to automate invoice discounts.</p>
                    </div>
                ) : (
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-500 mt-0.5">
                                            ...
                                            {s.medicine && <span className="ml-2 px-1.5 py-0.5 bg-slate-100 rounded text-[10px] font-mono">{s.medicine.name}</span>}
                                        </p>
                                    </div>
                                </div>
