                  ...
                  {groupItems.map(ci => (
                    <motion.div key={ci.medicine.id} layout exit={{ x: -300, opacity: 0 }} transition={{ duration: 0.2 }} className="flex items-center gap-3 p-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-slate-800 line-clamp-1">{ci.medicine.name}</p>
                        <p className="text-xs text-slate-400 font-medium">{ci.medicine.brand} · ₹{ci.medicine.price.toFixed(2)}/unit</p>
                      </div>
                      <div className="flex items-center gap-1 bg-slate-50 rounded-xl border border-slate-100/50">
                        <motion.button whileTap={{ scale: 0.9 }} onClick={() => ci.qty <= 1 ? removeItem(ci.medicine.id) : updateQty(ci.medicine.id, ci.qty - 1)}
                          className="p-2 hover:bg-slate-100 rounded-l-xl transition-colors"><Minus size={12} className="text-slate-500" /></motion.button>
