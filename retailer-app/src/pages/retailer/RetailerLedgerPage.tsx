...
export const RetailerLedgerPage: React.FC = () => {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Agency List */}
      <div>
        <h3 className="text-xs font-semibold text-slate-700 uppercase tracking-wide mb-3">Agencies</h3>
        <div className="space-y-2">
          {agencies.length === 0 ? (
            <div className="text-center py-8 bg-white rounded-xl border border-slate-100">
              ...
              <p className="text-sm text-slate-500">No agencies connected</p>
            </div>
          ) : (
            agencies.map((agency: any) => {
              const isSelected = selectedAgency === agency.wholesaler_id;
              return (
                <motion.div key={agency.wholesaler_id} layout className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
                  <button onClick={() => handleAgencyClick(agency.wholesaler_id)} className="w-full flex items-center gap-3 p-4 text-left hover:bg-slate-50 transition-colors">
                    <div className="w-10 h-10 rounded-xl bg-blue-50 flex items-center justify-center shrink-0">
                      <Building2 size={16} className="text-blue-600" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-slate-800">{agency.wholesaler_name || agency.name || 'Agency'}</p>
                      {agency.is_primary && <span className="text-[10px] font-semibold text-amber-600 bg-amber-50 px-1.5 py-0.5 rounded-full mt-0.5 inline-block">Primary</span>}
                    </div>
                    <div className="text-right">
                    ...
                    {isSelected ? <ChevronDown size={16} className="text-slate-400" /> : <ChevronRight size={16} className="text-slate-400" />}
                  </button>

                  {/* Transaction History */}
                  {isSelected && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} className="border-t border-slate-50">
                      {/* Filter */}
                      <div className="flex items-center gap-2 px-4 py-3 bg-slate-50 border-b border-slate-100">
                        <Filter size={12} className="text-slate-400" />
                        {['ALL', 'CREDIT', 'DEBIT'].map(f => (
                          <button key={f} onClick={() => setFilter(f as any)}
                            className={cn("text-[10px] font-medium px-2.5 py-1 rounded-full border transition-colors",
                          ...
                          {Object.entries(dateGroups).map(([date, entries]) => (
                            <div key={date}>
                              <div className="sticky top-0 px-4 py-2 bg-slate-50 border-b border-slate-100">
                                <span className="text-[10px] font-semibold text-slate-400 flex items-center gap-1"><Calendar size={10} />{date}</span>
                              </div>
                              {entries.map((entry: any) => (
                                <div key={entry.id} className="flex items-center gap-3 px-4 py-3 border-b border-slate-50 last:border-0">
                                  <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center", entry.type === 'DEBIT' ? "bg-rose-50" : "bg-emerald-50")}>
                                  ...
                                  <div className="flex-1 min-w-0">
                                    ...
                                    {entry.reference_id && <p className="text-[10px] text-slate-400">Ref: {entry.reference_id}</p>}
                                  </div>
                                  <div className="text-right">
                                    <p className={cn("text-sm font-semibold", entry.type === 'DEBIT' ? "text-rose-600" : "text-emerald-600")}>
                                      {entry.type === 'DEBIT' ? '+' : '-'}₹{entry.amount.toFixed(0)}
                                    </p>
                                    <p className="text-[10px] text-slate-400">Bal: ₹{entry.balance_after?.toFixed(0) || '—'}</p>
                                  </div>
                                </div>
                              ))}
                            </div>
